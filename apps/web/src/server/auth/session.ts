...
export async function validateSession(sessionToken: string) {
  ...
  const session = await prisma.session.findUnique({
    ...
    include: { user: { select: { id: true, email: true, role: true, emailVerifiedAt: true, displayName: true, bio: true, avatarUrl: true, tags: true, preferences: true, profileVisibility: true, emailDigestFrequency: true, aiProvider: true } } }
  });
