...
3. **Build** the `claude exec` command:
   - If `agent` is set, prefix the prompt with the agent file path
     (e.g., `--agent .claude/agents/longrunning-agent/AGENT.md`).
...
|-------------------------|-----------------------------------------------------|
| `longrunning-agent`     | `.claude/agents/longrunning-agent/AGENT.md`         |
| `orchestrator-agent`    | `.claude/agents/orchestrator-agent/AGENT.md`        |
